...
async function main() {
    ...
    http.createServer(async (req, res) => {
        ...
        if (req.url.startsWith("/api")) {
            ...
            if (typeof queryObject.puzzleid === "string") {
                if (queryObject.puzzleid.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i)) {

                    console.log(`Request puzzle: ${queryObject.puzzleid}`);
                    let startGetData = Date.now();
                    await axiosRequest.get(`https://ubistatic-a.akamaihd.net/0098/captcha/generated/${queryObject.puzzleid}-PuzzleWithMissingPiece.rttex`, {
                            headers: header
                        })
                        .then(async response => {
                            let endGotData = Date.now();
                            console.log(`Got puzzle: ${queryObject.puzzleid} in ${endGotData - startGetData}ms`);
                            ...
                            console.log(`Puzzle answer: ${queryObject.puzzleid} position ${apiResponse}`);

                            let endSolving = Date.now();
                            console.log(`Solved puzzle: ${queryObject.puzzleid} in ${endSolving - startSolving}ms`);
                        })
                        .catch(ex => {
                            console.log(`Failed to get puzzle: ${queryObject.puzzleid}`);
                            console.log(`${ex}`);
                        });
                }
            }
            ...
            do {
                await axiosRequest.get("https://growx.my.id/server", {
                })
                .then(async response => {
                    repeat = false;
                    console.log(`Data: ${response.data}`);
